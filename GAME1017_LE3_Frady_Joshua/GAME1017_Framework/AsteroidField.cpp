...
Asteroid::Asteroid(SDL_Rect s, SDL_FRect d, int i)  : Sprite(s, d),
	m_angle(0.0), m_radius(33.0), m_size(i)
{
	if(m_size==1)
	{
		m_dst.w *= 0.66f;
...
{
	SDL_SetTextureColorMod(TEMA::GetTexture("sprites"), 0-m_rMod, 255-m_gMod, 255-m_bMod);
...
void AsteroidField::asteroid_split(Asteroid* p_asteroid, double angle)
{
	//left asteroid chunk 
	Asteroid* temp = new Asteroid({ 563,0,61,66 }, { p_asteroid->GetCenter().x - 20.0f, p_asteroid->GetCenter().y, 61.0f, 66.0f },p_asteroid->GetSize() - 1);
	/*temp->SetColMods(p_asteroid->GetColMods());*/
	temp->UpdateDeltas(-angle); 
	m_asteroids.push_back(temp);
	temp->SetColMods(p_asteroid->GetColMods()[0], p_asteroid->GetColMods()[1], p_asteroid->GetColMods()[2]);

	//right Asteroid chunk
	temp = new Asteroid({ 563,0,61,66 }, { p_asteroid->GetCenter().x - 20.0f, p_asteroid->GetCenter().y, 61.0f, 66.0f }, p_asteroid->GetSize() - 1);
	//temp->SetColMods(p_asteroid->GetColMods());
	temp->UpdateDeltas(-angle);
	m_asteroids.push_back(temp);
	temp->SetColMods(p_asteroid->GetColMods()[0], p_asteroid->GetColMods()[1], p_asteroid->GetColMods()[2]);
