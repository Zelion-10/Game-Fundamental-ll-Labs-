...
void GameState::Update()
{
	...
	AsteroidField* field = static_cast<AsteroidField*>( GetChild("field"));
	...
	for (unsigned int i = 0; i < bullets.size(); i++)
	{
		...
		for (unsigned int j = 0; j < field->GetAsteroids().size(); j++)
		{
			Asteroid* ast = field->GetAsteroids().at(j);
			if (COMA::CircleCircleCheck(bul->GetCenter(), ast->GetCenter(),
				bul->GetRadius(), ast->GetRadius()))
			{
				// Spawn new asteroid chunks.
				Asteroid* AsteroidChild;
				if (ast->GetSize() > 0) // check if the asteroid is big enough to split
				{
					//Spawn of the asteriod chunks 
					field->asteroid_split(ast, ((rand()%  15 + 30) + bul->GetAngle()));
					//Colours, WOW COLOURS WE LOVE COLOURS   
					/*AsteroidChild = new Asteroid({ 539,0,61,66 }, { ast->GetCenter().x - 20.0f,ast->GetCenter().y,61,66 },2);*/
					/*AsteroidChild->SetSize(ast->GetSize() - 1);
					AsteroidChild->SetColMods(ast->GetColMods()[0], ast->GetColMods()[1], ast->GetColMods()[2]);
					AsteroidChild->UpdateDeltas(bul->GetAngle() - (30 + rand() % 16));
					if (ast->GetSize() == 1)
					{
						AsteroidChild->GetDst()->w *= 0.66f;
						AsteroidChild->GetDst()->h *= 0.66f;
						AsteroidChild->GetRadius() *= 0.66;

					}
					else if (ast->GetSize() == 0)
					{
						AsteroidChild->GetDst()->w *= 0.33f;
						AsteroidChild->GetDst()->h *= 0.33f;
						AsteroidChild->GetRadius() *= 0.33;
					}
					field->GetAsteroids().push_back(AsteroidChild);
					*/
				}
